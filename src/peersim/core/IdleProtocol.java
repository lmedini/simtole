...
public void setNeighbor(int i, Node newNeighbour) {
	...
	neighbors[i]=newNeighbour;
}

public boolean removeNeighbour(Node neighbour) {
	//Recherche de l'index du voisin
	int index = -1;
	for (int i = 0; i < len; i++) {
		if (neighbors[i] == neighbour)
			index = i;
	}
	// Si pas d'index trouvé, retour
	if(index == -1)
		return false;
	// Si index trouvé, décalage de la fin du tableau
	for (int i = index + 1; i < len; i++) {
		neighbors[i-1] = neighbors[i];
	}
	len--;
	// Suppression du dernier élément (en double)
	neighbors[len] = null;
	return true;
}
}
