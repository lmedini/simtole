 ...
 * {@link Control}. They are run <b>only once</b>, in
...
/**
 * This is the prefix for simulation need initializers. These have to be of type
 * {@link Control}. They are run at the beginning of each experiment, in
 * the order specified by the configuration.
 * @see Configuration
 * @config
 */
private static final String PAR_SIMNEEDINIT = "simulneedinit";
...
private static void runInitializers()
{
	...
	//System.err.println("CDSim initializer: " + inits.length + " inits, " + names.length + " names");
...
/**
 * Load and run initializers that will be loaded each time a simulation begins (Lionel).
 */
private static void runSimulationNeedInitializers()
{

	Object[] inits = Configuration.getInstanceArray(PAR_SIMNEEDINIT);
	String names[] = Configuration.getNames(PAR_SIMNEEDINIT);

	//System.err.println("CDSim initializer: " + inits.length + " inits, " + names.length + " names");
	for (int i = 0; i < inits.length; ++i) {
		System.err.println("- Running simulation need initializer " + names[i] + ": " + inits[i].getClass());
		((Control) inits[i]).execute();
	}
}
...
/**
 * Initialization, taken out from nextExperiment (Lionel)
 */
...
/**
 * Runs an experiment, without resetting anything (Lionel). 
 */
public static final void nextExperiment()
{
	System.err.println("CDSimulator: running simulation need initializers");
	runSimulationNeedInitializers();
	...
	for (int i = 0; i < cycles; ++i) {
		...
		System.out.println("\n-------------------------------------------------------------------");
		System.out.println("--------------------- CDSimulator: cycle " + i + " ------------------------");
