...
public static Graph wireXMLTopology ( Graph g , String urlXML){
	//ParserXML px = new ParserXML(g.size());
	TopologyHandler px = new TopologyHandler();
	ArrayList<ArrayList<Integer>> tableNode;
	ArrayList<Integer> neighbors;
	
	try {
		tableNode = px.parseXMLTopology(urlXML);
		System.out.println("Lecture de la topologie dans le fichier XML " + urlXML);
		for(int i=0;i<tableNode.size();i++){
			neighbors = tableNode.get(i);
			for (int j=0;j<neighbors.size();j++){
				//System.out.println("node " + i + " has the neighbors " + neighbors.elementAt(j));
				g.setEdge(i, neighbors.get(j));	
			}
		}
...
/**
 * Unoptimized method for resetting the graph topology.
 * @param g Graph tp be resetted
 * @param size Number of nodes in the graph
 */
public static void unWireTopology (Graph g, int size){
